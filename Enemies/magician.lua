--- Magician enemy: A flying mage that casts projectiles at the player.
...
local PROJECTILE_DODGE_RANGE = 4  -- Tiles: detect incoming projectiles within this range (needs buffer for frame timing)
...
magician.animations = {
	...
	BOLT_HIT = Animation.create_definition(sprites.enemies.magician.projectile_hit, 5, {
		ms_per_frame = 60, width = 16, height = 16, loop = false
	}),
}
...
--- Check if a raycast from a point hits solid geometry (shared by is_path_clear and find_fly_position)
...
--------------------------------------------------------------------------------
-- Bolt trail particle pool (local to this module)
--------------------------------------------------------------------------------
local BoltParticle = {}
BoltParticle.all = {}
...
--------------------------------------------------------------------------------
-- Puff particle effect (disappear effect)
--------------------------------------------------------------------------------
local PuffParticle = {}
PuffParticle.all = {}
PuffParticle.count = 0

local PUFF_PARTICLE_COUNT = 50       -- Number of particles in puff (reduced from 100)
...
local PUFF_POOL_SIZE = 200           -- Pre-allocated puff pool size (supports multiple concurrent puffs)
...
--------------------------------------------------------------------------------
-- MagicBolt projectile pool (local to this module)
--------------------------------------------------------------------------------
local MagicBolt = {}
MagicBolt.all = {}
...
function MagicBolt.spawn(x, y, target_x, target_y)
	local dx = target_x - x
	local dy = target_y - y
	...
	local bolt = {
		...
		vy = vy,
		box = BOLT_BOX,
		animation = Animation.new(magician.animations.BOLT, bolt_anim_opts),
...
local function bolt_impact(bolt)
	...
	Effects.new("magic_bolt_hit", magician.animations.BOLT_HIT, effect_x, effect_y)
...
--- Check for wall-stuck and projectile-dodge interrupts (used by combat states)
---@param enemy table The magician enemy
---@return boolean True if state was interrupted
local function check_combat_interrupts(enemy)
	if (enemy._wall_check_timer or 0) <= 0 then
		enemy._wall_check_timer = WALL_CHECK_INTERVAL
		if is_in_wall(enemy) then
			enemy:set_state(magician.states.unstuck)
			return true
		end
	end
	if should_dodge_projectile(enemy) then
		enemy:set_state(magician.states.disappear)
		return true
	end
	return false
end
...
local function find_safe_position(enemy)
	...
	for _, dist in ipairs(SAFE_POSITION_DISTANCES) do
		for _, angle in ipairs(SAFE_POSITION_ANGLES) do
			...
			if is_position_clear(test_shape) then
...
--- Prefers staying on the same side, only crosses to opposite if necessary
---@param enemy table The magician enemy
...
--- Decide next state after fly/attack based on player position and LOS
---@param enemy table The magician enemy
local function choose_next_combat_state(enemy)
	if not enemy.target_player then
		enemy:set_state(magician.states["return"])
		return
	end

	if common.player_in_range(enemy, ATTACK_RANGE) and common.has_line_of_sight(enemy) then
		enemy:set_state(magician.states.attack)
	elseif not common.player_in_range(enemy, TOO_FAR_DISTANCE) then
		enemy:set_state(magician.states.disappear)
	elseif not common.has_line_of_sight(enemy) then
		enemy:set_state(magician.states["return"])
	else
		enemy:set_state(magician.states.fly)
	end
end
...
magician.states.idle = {
	name = "idle",
	start = function(enemy, _)
		begin_state(enemy, magician.animations.IDLE)
		enemy.alpha = 1
...
magician.states.attack = {
	name = "attack",
	start = function(enemy, _)
		begin_state(enemy, magician.animations.ATTACK)
		enemy.bolt_spawned = false
		face_player(enemy)

		-- Store target position at attack start (projectile aims at current player position)
	...
	update = function(enemy, dt)
		...
		if not enemy.bolt_spawned and enemy.animation.frame >= 10 then
			...
			-- Spawn from top corner based on direction (right side when facing right, left when facing left)
...
magician.states.disappear = {
	...
	start = function(enemy, _)
		...
		if enemy.target_player then
			enemy._start_side = enemy.x < enemy.target_player.x and -1 or 1  -- -1 = left, 1 = right
		else
			enemy._start_side = 1
		end
	...
	update = function(enemy, dt)
		...
		elseif enemy.phase == "fade_in" then
			...
			if enemy.fade_timer >= FADE_IN_DURATION then
				...
				enemy.invulnerable = false
				combat.add(enemy)
				enemy:set_state(magician.states.idle)
			end
		end
	end,
	draw = draw_magician,
...
magician.states.hit = {
	name = "hit",
	start = function(enemy, _)
		begin_state(enemy, magician.animations.HIT)
		enemy.invulnerable = true
	end,
	update = function(enemy, dt)
		update_common(enemy, dt)
...
--------------------------------------------------------------------------------
-- Custom on_hit handler (no knockback/stun)
--------------------------------------------------------------------------------
...
local function magician_on_hit(self, _source_type, source)
	...
	if self.health <= 0 then
		...
		self:set_state(magician.states.hit)
	end
end
...
return {
	...
	animations = magician.animations,
	initial_state = "idle",
	on_hit = magician_on_hit,
