...
Prop.register("boss_block", require("Prop/boss_block"))
...
local function update(dt)
    ...
    if player.is_dead and not was_dead then
        was_dead = true
        -- Increment death count and persist alongside playtime so they aren't lost on reload
        player.deaths = (player.deaths or 0) + 1
        if active_slot then
            local save_data = SaveSlots.get(active_slot)
            if save_data then
                save_data.deaths = player.deaths
